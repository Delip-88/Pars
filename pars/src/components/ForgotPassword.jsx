...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const data = await fetch("http://localhost:4000/api/auth/reset-password", {
      ...
      headers: {
        ...
        body: JSON.stringify({ email: localStorage.getItem("emailForVerification") || "", token: id, newPassword: formData.password }),
    })
  ...
  const getPasswordStrength = (password) => {
    ...
    if (strength === 100) return { width: "100%", color: "bg-gradient-to-r from-rose-500 to-rose-600" }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-blue-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="text-center text-3xl font-bold bg-gradient-to-r from-rose-600 to-blue-600 bg-clip-text text-transparent">
          Set New Password
        </h2>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <label htmlFor="password" className="block text-sm font-semibold text-gray-700 mb-2">
                <Lock className="w-4 h-4 inline mr-2 text-rose-600" />
                New Password
              </label>
              <div className="relative">
                ...
                <button
                  type="button"
                  className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400 hover:text-rose-600 transition-colors cursor-pointer"
                ...
                <div className="space-y-2">
                  {requirements.map((requirement, index) => (
                    <div key={index} className="flex items-center text-sm">
                      ...
                      <span
                        className={`${requirement.regex.test(formData.password) ? "text-rose-700" : "text-gray-600"}`}
                      >
                        {requirement.text}
                      </span>
                    </div>
              ...
              {formData.confirmPassword && formData.password && (
                <div className="mt-2 flex items-center text-sm">
                  {formData.password === formData.confirmPassword ? (
                    <>
                      <div className="w-5 h-5 rounded-full bg-rose-100 flex items-center justify-center mr-2">
                        <Check className="h-3 w-3 text-rose-600" />
                      </div>
                      <span className="text-rose-700">Passwords match</span>
                    </>
                  ) : (
                    <>
            ...
            <div className="pt-4">
              <button
                ...
                className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-xl shadow-lg text-sm font-semibold text-white bg-gradient-to-r from-rose-600 to-rose-700 hover:from-rose-700 hover:to-rose-800 focus:outline-none focus:ring-4 focus:ring-rose-200 focus:ring-opacity-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-[1.02]"
              >
          ...
          <div className="mt-6 p-4 bg-rose-50 rounded-xl border border-rose-100">
            <div className="flex items-start">
              <div className="flex-shrink-0">
                <Shield className="h-5 w-5 text-rose-600 mt-0.5" />
              </div>
              <div className="ml-3">
